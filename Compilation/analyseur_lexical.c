...
char *tableMotsClefs[] = {
  "si","alors", "retour", "sinon","tantque", "faire","entier","lire","ecrire",
};
...
char * tableauSymboleOperation [] = {
  "+", "-", "*", "/", "(", ")", "[", "]", "{", "}", ",", ";", "=", "<", "&", "|", "!",
};

int codeSymboleOperation[] = {
  PLUS, MOINS, FOIS, DIVISE, PARENTHESE_OUVRANTE, PARENTHESE_FERMANTE, CROCHET_OUVRANT, CROCHET_FERMANT,
  ACCOLADE_OUVRANTE, ACCOLADE_FERMANTE, VIRGULE, POINT_VIRGULE, EGAL, INFERIEUR, ET, OU, NON,
};

char yytext[YYTEXT_MAX];
int yyleng;
int nbMotsClefs = 9;
int nbSymboleOperation = 17;
...
int yylex(void)
{
  ...
  //symbole
  if(!is_alphanum(c)){
    for(int j = 0; j < nbSymboleOperation; j++){
      if(strcmp(yytext, tableauSymboleOperation[j]) == 0){
        return codeSymboleOperation[j];
      }
    }
  }

  //mot CLEF et Fonction et var
  if(is_alpha(c)){
    if(c == '$'){
      while(is_alphanum(c)){
        c = lireCar();
      ...
      delireCar();
      for(int w = 0; w < nbMotsClefs; w++){
        if(strcmp(yytext, tableMotsClefs[w]) == 0){
          return codeMotClefs[w];
        }
      }
    }
  }
